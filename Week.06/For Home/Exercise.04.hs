...
roundResult number = (fromIntegral $ round $ number * 100) / 100 

getAverageBalance :: ([Account],[Person]) -> (Person -> Bool) -> Double
getAverageBalance (accounts, people) f = roundResult result
 where
    filteredPeople = filter (\ x -> f x ) people
    filteredIds = map (\ (id, _, _) -> id ) $ filteredPeople
    salaries = map (\ (_, _, salary) -> salary) $ filter (\ (_, id, salary) -> elem id filteredIds) accounts
    result = sum salaries / (fromIntegral $ length salaries)

averageBalanceOfCities :: ([Account],[Person]) -> [String] -> Double
averageBalanceOfCities (accounts, people) cities = roundResult result
 where
    filteredPeople = filter (\ (_, _, city) -> elem city cities) people
    filteredIds = map (\ (id, _, _) -> id ) $ filteredPeople
    salaries = map (\ (_, _, salary) -> salary) $ filter (\ (_, id, salary) -> elem id filteredIds) accounts
    result = sum salaries / (fromIntegral $ length salaries)